...
def generate_explanation(exp_args):
    ...
    base_model.load_state_dict(torch.load(os.path.join("logs", exp_args.dataset+"_logs", "model.model")))
    base_model.eval()
    #  fix the rec model
    ...
    with open(os.path.join(exp_args.save_path, exp_args.dataset + "_optimization_obj.pickle"), 'wb') as outp:
